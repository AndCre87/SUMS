...
g <- c(2,4,2,3,2) #Simulate age and sex without intercept (it's the lambda_tilde!)
X <- list()
...
X[[3]] <- matrix(c((runif(N) <= 0.75), (runif(N) <= 0.25)), N, g[3])

X[[4]] <- matrix(rgamma(N*g[4], 0.1, 10), N, g[4])

X[[5]] <- matrix(rt(N*g[5], 25), N, g[5])
...
for(h in 1:pY){
  ...
  # # For testing without constant covariates
  # beta_simul[[h]] <- matrix(0,g[h],dY[h]*(dY[h] - 1),byrow = TRUE)
}
...
for(h in 1:pY){
  for(i in 1:N){
    ...
    Z1 <- rnorm(h/pY*times_i/T_max, sd = sqrt(0.5))
    Z2 <- rnorm(h + cos(times_i/T_max*2*pi), sd = sqrt(0.5))
...
for(h in 1:pY){
  ...
  #For testing without time-varying continuous covariates
  gamma_simul[[h]] <- matrix(0,q[h],dY[h]*(dY[h] - 1),byrow = TRUE)
}
...
#Response process (_obs contains missing values at time 0)
Y_true <- vector("list", length = N)
...
for(i in 1:N){
  ...
  for(h in 1:pY){
    ...
    #Sample initial value from the stationary distribution
    #In this simulation we assume processes with two states only!
    lambda_start <- lambda_Y_msm_i_h * exp( Z_msm_i_h[1,] %*% gamma_simul_h )
    
    Y_start <- 2
    if(runif(1) <= lambda_start[2] / sum(lambda_start)){
      Y_start <- 1
    }
    
    Y_i_h_msm <- sim.msm(qmatrix = Q_Y, maxtime = T_max, covs = Z_msm_i_h, beta = gamma_simul_h, obstimes = times_i_h, start = Y_start, mintime = 0)
    ...
    Y_true_i[[h]] <- Y_i_h_msm$states[c(1,ind-1,length(Y_i_h_msm$times))] - 1 #(0,1)
    Y_i[[h]] <- Y_true_i[[h]]
    
    #A small percentage of missing values at time 0
    if(runif(1) <= 0.1){
      Y_i[[h]][1] <- NA
    }
  }
...
if(pH > 0){
  ...
  for(i in 1:N){
    ...
    H_true_i <-vector("list", length = pH)
    ...
    for(l in 1:pH){
      ...
      #In this simulation we assume processes with two states only!
      H_start <- 2
      if(runif(1) <= lambda_H_simul_i_l[2] / sum(lambda_H_simul_i_l)){
        H_start <- 1
      }
      
      H_i_l_msm <- sim.msm(Q_H, maxtime = T_max, covs = NULL, beta = NULL, obstimes = times_i_l, start = H_start, mintime = 0)
      
      #Exclude first and last time points
      out_aux <- outer(times_i_l[2:(n_times_i_l-1)], H_i_l_msm$times, "<=")
      ind <- apply(out_aux, 1, function(x){which(x)[1]})
      
      H_true_i[[l]] <- H_i_l_msm$states[c(1,ind-1,length(H_i_l_msm$times))]  - 1 #(0,1)
      H_i[[l]] <- H_true_i[[l]]
...
if(pH > 0){
  data <- list(Y = Y, H = H, n_states = n_states, n_rates = n_rates, n_times = n_times, epsilon = epsilon, X = X)
}else{
  data <- list(Y = Y, n_states = n_states, n_rates = n_rates, n_times = n_times, epsilon = epsilon, X = X)
}
...
Alg_list <- list(n_save = n_save, n_burn1 = n_burn1, n_burn2 = n_burn2, thin = thin, n_edges = n_edges, threshold = threshold, SM_alg = SM_alg, t_SM_split = 5, t_SM_merge = 5)
...
#Check missing values recovery
Y_out <- sample.bdmcmc$Y_out
for(i in 1:N){
  Y_true_i <- Y_true[[i]]
  Y_i <- Y[[i]]
  for(h in 1:pY){
    Y_true_i_h <- Y_true_i[[h]]
    Y_i_h <- Y_i[[h]]
    
    if(is.na(Y_i_h[1])){
      
      Y_out_i_h <- rep(0,n_save)
      for(it in 1:n_save){
        Y_out_i_h[it] <- Y_out[[it]][[i]][[h]][1]
      }
      
      print(paste("Y_true_", i, "_", h, " = ", Y_true_i_h[1], sep = ""))
      print(paste("P(Y_", i, "_", h, " | all) =  ", sep = ""))
      print(table(Y_out_i_h)/n_save)
      print("")
    }
  }
}
...
if(pH > 0){
  ...
  for(i in 1:N){
    ...
    H_i <- H[[i]]
    for(l in 1:pH){
      H_true_i_l <- H_true_i[[l]]
      H_i_l <- H_i[[l]]
      
      if(is.na(H_i_l[1])){
        
        H_out_i_l <- rep(0,n_save)
        for(it in 1:n_save){
          H_out_i_l[it] <- H_out[[it]][[i]][[l]][1]
        }
        
        print(paste("H_true_", i, "_", l, " = ", H_true_i_l[1], sep = ""))
        print(paste("P(H_", i, "_", l, " | all) =  ", sep = ""))
        print(table(H_out_i_l)/n_save)
        print("")
      }
    }
  }
...
for(it in 1:n_save){
  ...
  for(i in 1:N){
    ...
    for(h in 1:pY){
      ...
      lambda_Y_i[[h]] <- matrix(exp( phi_star_it_out[c_out[it,i]+1,ind_dY] + X_h[i,] %*% beta_it_h_out), nrow = n_times_i_h-1, ncol = dY[h]*(dY[h]-1), byrow = TRUE) * exp( Z_h_i[2:n_times_i_h,] %*% gamma_it_h_out )
    }
...
for(i in 1:N){
  ...
  for(h in 1:pY){
    ...
    gamma_simul_h <- gamma_simul[[h]]
    lambda_Y_simul_i[[h]] <- matrix(lambda_Y_tilde_simul[[c_simul[i]]][[h]] * exp( X_h[i,] %*% beta_simul_h ), n_times_i_h - 1, dY[h] * (dY[h] - 1), byrow = TRUE) * exp( Z_h_i[2:n_times_i_h,] %*% gamma_simul_h )
  }
...
phi_star_out <- sample.bdmcmc$phi_star_out
c_out <- sample.bdmcmc$c_out
beta_out <- sample.bdmcmc$bXZ_List$beta_out
...
for(it in 1:n_save){
  ...
  for(i in 1:N){
    ...
    for(h in 1:pY){
      ...
      lambda_Y_i[[h]] <- matrix(exp( phi_star_it_out[c_out[it,i]+1,ind_dY] + X_h[i,] %*% beta_it_h_out), nrow = n_times_i[h]-1, ncol = dY[h]*(dY[h]-1), byrow = TRUE)
    }
    lambda_H_i <- vector("list", length = pH)
    for(l in 1:pH){
      ind_dH <- (n_rates_cum[pY+l]+1):n_rates_cum[pY+l+1]
...
for(i in 1:N){
  ...
  for(h in 1:pY){
    ...
    Z_h_i <- Z_h[[i]]
    beta_simul_h <- beta_simul[[h]]
    lambda_Y_simul_i[[h]] <- matrix(lambda_Y_tilde_simul[[c_simul[i]]][[h]] * exp( X_h[i,] %*% beta_simul_h ), n_times_i_h - 1, dY[h] * (dY[h] - 1), byrow = TRUE)
  }
  lambda_Y_simul[[i]] <- lambda_Y_simul_i
}
...
for(i in 1:N){
  lambda_Y_msm_i <- list()
  for(h in 1:pY){
    lambda_Y_msm_i[[h]] <- lambda_Y_tilde_simul[[c_simul[i]]][[h]]
  }
  lambda_Y_msm[[i]] <- lambda_Y_msm_i
}
...
for(i in 1:N){
  ...
  for(h in 1:pY){
    ...
    Z_h_i <- Z_h[[i]]
    gamma_simul_h <- gamma_simul[[h]]
    lambda_Y_simul_i[[h]] <- matrixexp( Z_h_i %*% gamma_simul_h )
  }
  lambda_Y_simul[[i]] <- lambda_Y_simul_i
}
