...
List DisProgrGibbs_DP(List MCMC_input){
  ...
  arma::vec d_out(n_save, arma::fill::zeros), size_G0_out(n_save, arma::fill::zeros), sum_weights_out(n_save,arma::fill::zeros), K_N_out(n_save,arma::fill::zeros), k0_out(n_save, arma::fill::zeros), alpha_out(n_save, arma::fill::zeros), lambda_out(n_save,arma::fill::zeros), mu_d_out(n_save,arma::fill::zeros), Entropy_out(n_save, arma::fill::zeros);
  ...
  for(int it = 0; it < n_tot; it++){
    ...
    if(update_c){
      ...
      for(int i = 0; i < N; i++){ // Allocation of i-th subject
        ...
        f_k1.head(N_aux) = logLike_all(setC1, Y, H, phi_star_aux, n_times, epsilon, X, Z, beta, gamma, n_rates_cum, dY, dH, impute_missing_Y, impute_missing_H);
        // Probability of allocating to an existing cluster
        f_k1.tail(K_N) = logLike_all(setC1, Y, H, phi_star, n_times, epsilon, X, Z, beta, gamma, n_rates_cum, dY, dH, impute_missing_Y, impute_missing_H);
    ...
    for(int j = 0; j < K_N; j++){ //Need an integer here
      ...
      num_loglike = arma::accu(logLike_all(setCm, Y, H, phi_star_row_new, n_times, epsilon, X, Z, beta, gamma, n_rates_cum, dY, dH, impute_missing_Y, impute_missing_H));
      den_loglike = arma::accu(logLike_all(setCm, Y, H, phi_star.row(j), n_times, epsilon, X, Z, beta, gamma, n_rates_cum, dY, dH, impute_missing_Y, impute_missing_H));
  ...
  if(impute_missing_Y){
    if(impute_missing_H){
      return List::create(Named("Graph_List") = Graph_List, Named("mu_out") = mu_out, Named("m_mu_out") = m_mu_out, Named("k0_out") = k0_out, Named("phi_star_out") = phi_star_out, Named("c_out") = c_out, Named("K_N_out") = K_N_out, Named("alpha_out") = alpha_out, Named("XZ_List") = XZ_List, Named("Binder_List") = Binder_List, Named("Y_out") = Y_out, Named("H_out") = H_out);
    }else{
    ...
    if(impute_missing_H){
      return List::create(Named("Graph_List") = Graph_List, Named("mu_out") = mu_out, Named("m_mu_out") = m_mu_out, Named("k0_out") = k0_out, Named("phi_star_out") = phi_star_out, Named("c_out") = c_out, Named("K_N_out") = K_N_out, Named("alpha_out") = alpha_out, Named("XZ_List") = XZ_List, Named("Binder_List") = Binder_List, Named("H_out") = H_out);
    }else{
