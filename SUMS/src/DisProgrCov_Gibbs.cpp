...
List DisProgrCov_Gibbs(List MCMC_input){
  ...
  if(size_based_prior){
    a_eta = Param_list["a_d"], b_eta = Param_list["b_eta"];
    eta = 0.5; // Initialize
  }else{
    ...
    if(!size_based_prior){
      if(update_eta){
        if(eta_beta_prior){
          ...
          s_eta = s_eta + pow(g+1,-ADAPT(1))*(accept_eta - ADAPT(2));
          if(s_eta > exp(50)){
            s_eta = exp(50);
          }else{
            if(s_eta < exp(-50)){
  ...
  return List::create(Named("Omega_out") = Omega_out, Named("G_out") = G_out, Named("G0_out") = G0_out, Named("sizeG0_out") = size_G0_out, Named("sum_weights_out") = sum_weights_out, Named("mu_out") = mu_out, Named("m_mu_out") = m_mu_out, Named("eta_out") = eta_out, Named("k0_out") = k0_out, Named("mu_eta_out") = mu_eta_out, Named("lambda_out") = lambda_out);
}
